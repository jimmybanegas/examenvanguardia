module.exports = class WordChain {
  ...
  findPath(startWord, endWord, theWordList) {
    ...
    while (connectedSet.length > 0) {
      ...
      for (let i = 0; i < wordList.length; i += 1) {
        if (this.isConnected(node.word, wordList[i])) {
          const newPath = node.path.concat(wordList[i]);
